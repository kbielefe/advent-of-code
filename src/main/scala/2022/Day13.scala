...
object Day13:
  ...
  def part1(input: MultiLine[List[Json]]): Int =
    input.zipWithIndex.filter{case (List(left, right), index) => left < right}.map(_._2 + 1).sum

  def part2(input: MultiLine[List[Json]]): Int =
    val dividerPackets = List("[[2]]", "[[6]]").map(parse).flatMap(_.toOption)
    (dividerPackets ++ input.flatten).sorted.zipWithIndex.filter((packet, _) => dividerPackets.contains(packet)).map(_._2 + 1).product
