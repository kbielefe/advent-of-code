...
class Day5(source: Source) extends Day {
  ...
  def containsDouble(in: String): Boolean = in.toSeq.sliding(2).exists{x => x(0) == x(1)}

  def containsForbiddenStrings(in: String): Boolean =
    in.toSeq.sliding(2).exists{Set("ab", "cd", "pq", "xy") contains _.toString}
  ...
  def hasNonOverlappingPairs(in: String): Boolean = {
    val pairsWithIndex = in.toSeq.sliding(2).zipWithIndex.toList
    val indicesOnly = pairsWithIndex.groupBy{_._1}.values.map{_.map{_._2}}
    indicesOnly.exists{x => x.max - x.min > 1}
  }

  def hasRepeatBetween(in: String): Boolean = in.toSeq.sliding(3).exists{x => x(0) == x(2)}
