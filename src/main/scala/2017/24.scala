...
class Day24(source: Source) extends Day {
  ...
  val input = rawInput.flatMap{case Array(x, y) => Set(Array(x, y), Array(y, x))}.groupBy{_(0)}.view.mapValues(_.map(_.sorted).map{case Array(x, y) => (x, y)})
  ...
  def bridges(input: Map[Int, Components], acc: Bridge = List.empty[(Int, Int)])(port: Int): LazyList[Bridge] = {
    if (input contains port) {
      input(port).to(LazyList)
        .flatMap{case (x, y) =>
          val next = bridges(removeElement((x, y), input), (x, y) :: acc) _
          if (x == port) next(y) else next(x)
        }
    } else {
      LazyList(acc.reverse)
    }
  }

  def strength(bridge: Bridge): Int = bridge.map{case (x, y) => x + y}.sum

  val memBridge = bridges(input.toMap)(0).map(x => (x.length, strength(x)))
