...
class Day14(source: Source) extends Day {
  ...
  val squares = (0 to 127)
    ...
    override def answer1 = squares.size.toString

    @tailrec
    private def region(toVisit: Set[(Int, Int)], result: Set[(Int, Int)] = Set.empty[(Int, Int)]): Set[(Int, Int)] = {
      if (toVisit.isEmpty) {
        result
      } else {
    ...
    @tailrec
    private def regionCount(squares: Set[(Int, Int)], count: Int = 0): Int = {
      if (squares.isEmpty) {
        count
      } else {
    ...
    override def answer2 = regionCount(squares).toString
}
