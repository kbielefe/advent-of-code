...
object Day11:
  ...
  case class Position(floors: Vector[Set[String]], floor: Int):
    def goal: Boolean = floors(0).isEmpty && floors(1).isEmpty && floors(2).isEmpty
  ...
  private def heuristic(start: Position): Int =
  ...
  val astar = new AStar[Position, Int](_.goal, heuristic, edgeWeight, 0, getNeighbors)

  def part1(input: List[String]): Int =
    astar.getMinCost(initialPosition).get

  def part2(input: List[String]): Int =
    astar.getMinCost(part2initialPosition).get
